...
### ⚡ 高级功能
- **动态配置路径**: 自定义模块路径，不限于默认位置
- **智能模块检测**: 基于配置路径的自动模块状态检测
- **全局异常处理**: 集中式错误管理，提高应用稳定性
- **错误日志系统**: 本地日志记录和安全分享功能
- **隐藏调试功能**: 开发者工具和错误分析支持
...
```bash
# 克隆项目
git clone https://github.com/reddoctor/ThreadUI.git
cd ThreadUI
...
├── ThreadUIApplication.kt               # 应用程序类（全局异常处理）
├── data/
│   ├── GameConfig.kt                   # 游戏配置数据模型
│   └── ShareConfig.kt                  # 分享数据模型
├── ui/
│   ├── components/                     # UI组件
│   │   ├── AboutDialog.kt              # 关于对话框（QQ群集成）
│   │   ├── SettingsDialog.kt           # 设置对话框（路径配置）
│   │   ├── GameEditDialog.kt           # 游戏编辑对话框
│   │   ├── ShareImportDialogs.kt       # 分享导入对话框
│   │   └── AppSelectorDialog.kt        # 应用选择对话框
│   └── theme/                          # Material 3 主题配置
└── utils/
    ├── RootUtils.kt                    # Root权限工具类
    ├── ShareUtils.kt                   # 文件分享工具类
    ├── ImportUtils.kt                  # 安全导入工具类
    ├── ConfigManager.kt                # 配置路径管理
    ├── ErrorLogger.kt                  # 错误日志系统
...
**注意**: 虽然本项目采用宽松的MIT许可证，但请合理使用Root权限功能，确保设备安全。
